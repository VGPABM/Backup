...
bool isCenteredRatio(double cx, double cy, 
    ...
    return isClose(object.x, cx, tx * 0.60) && isClose(object.y, cy, ty * 0.60);
...
//                    MATH STUFF                                        //
//=======================================================================
constexpr double vx_max = 1.00;                     // velocity maximum when line is align
...
void handleLine(geometry_msgs::Twist &vel, vision::RotatedRect &line_guide, double mult_x=1.0){
    ...
    // maju mundur
    // vel.linear.x = line_activation_x(vel.angular.z, mult_x) + 0.3;
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    int gatePhase = 0;
    double line_vel_x_multiplier[2] = {1.75, 1.0};// 0->for not islookingforhgate 1-> is for islookingforhgate
    ...
    while(ros::ok() && isDoingMission){
        ...
        // Cek horizontal gate                                  // check for false positive
            ...
            if(isCenteredRatio(camera_width/2, camera_height/2,  horizontalGate, final_ratio_x, final_ratio_y)){
    ...
    soero::Log::Info("Time = %lf", total_time);
