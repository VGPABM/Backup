...
PID mypid_z(dt, max_val, min_val, Kp+4.5, Kd+0.002, Ki);
PID mypid_z_hrz(dt, max_val+1.5, min_val-1.5, Kp+5.5, Kd+1, Ki+0.01);
// PID mypid_yaw(dt, 1.5, -1.5, 0.900, 0.100, 0.00000);
PID mypid_yaw(dt, 4.5, -4.5, 5.5, 1.55, 0.00000);
/*----------------------del time, interval val, kp,    kd,    ki*/
PID mypid_z_vision_gate(dt, max_val, min_val, 0.0095, 0.001, 0);
PID mypid_y_vision_gate(dt, max_val, min_val, 0.0040, 0.0016, 0);
PID mypid_y_hor_gate(dt, max_val, min_val, 0.500, 0.01, 0);
PID mypid_x_hor_gate(dt, max_val, min_val, 0.500, 0.01, 0);
...
bool isCenteredRatio(double cx, double cy, 
    ...
    // return isClose(object.x, cx, tx) && isClose(object.y, cy, ty);
    // return isClose(object.x, cx, tx) && isClose(object.y, cy, ty);
    // soero::Log::Info("x_diff: %f", (std::fabs(object.x-cx)));
    return isClose(object.y, cy, ty/2  && isClose(object.x, cx, 150));
}

bool isCenteredRatioHrz(double cx, double cy, 
                     vision::RotatedRect &object,double ratio_x=final_ratio_x, double ratio_y=final_ratio_y){
    double tx = object.w*ratio_x;
    double ty = object.h*ratio_y;
    soero::Log::Info("dx: %f | dy: %f", object.x-tx, object.y-ty);
    // return isClose(object.x, cx, tx) && isClose(object.y, cy, ty);
    return isClose(object.x, cx, tx) && isClose(object.y, cy, ty);
    // return isClose(object.y, cy, ty/2);
}
...
//                    MATH STUFF                                        //
//=======================================================================
constexpr double vx_max = 1.5;                     // velocity maximum when line is align
...
void handleLine(geometry_msgs::Twist &vel, vision::RotatedRect &line_guide, double mult_x=1.0){
    ...
    vel.angular.z = velAlignYawToLine(line_rad) * 1.1;
    soero::Log::Info("target is: %f, yet we are here: %f, so ang.z: %f", HALF_PI, line_rad, vel.angular.z);
    // maju mundur
    // vel.linear.x = line_activation_x(vel.angular.z, mult_x);
    if(line_guide.angle <= upper_threshold || line_guide.angle >= lesser_threshold){ //around 180 deg
        vel.linear.x = 0.6;
    }
    else{
        vel.linear.x = 0.3;
    ...
    // kiri kanan
    vel.linear.y = calc_vel_based_vision_z(line_guide.x, camera_width, get_max_width_meter(),mypid_y, 'y');
...
void handleLandingPad(geometry_msgs::Twist &vel, double hold_yaw, double hold_z, bool withTurun){
    ...
    vel.linear.y = calc_vel_based_vision_z(landingPad.x, camera_width, max_meter, mypid_y_hor_gate, 'y')*1.2;        
    vel.linear.x = calc_vel_based_vision_z(landingPad.y, camera_height, max_meter*(camera_height/camera_width), mypid_x_hor_gate, 'x')*1.5;//agak majuan sikit, biar masuk
...
void handleVerticalGate(geometry_msgs::Twist &vel){
    ...
    // centerin
    // vel.linear.y = mypid_y_vision_gate.calculate(camera_width/2, verticalGate.x);
    vel.linear.z = mypid_z_vision_gate.calculate(camera_height/2, verticalGate.y);
    // fix gate side
    handleVGateSide(vel);
...
void handleHorizontalGate(geometry_msgs::Twist &vel, double backup_yaw){
    ...
    vel.linear.y = calc_vel_based_vision_z(horizontalGate.x, camera_width, max_meter,mypid_y_hor_gate, 'y');
    vel.linear.x = calc_vel_based_vision_z(horizontalGate.y, camera_height, max_meter*(camera_height/camera_width), mypid_x_hor_gate, 'x');
}
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    bool isFirstCenteringLand=true;
    double lastHgateZ = 6;
    ...
    ros::Time firstLandingCentering;
    ros::Time firstAppereanceGate = ros::Time::now();
    ros::Time start_time = ros::Time::now();
    ros::Time start_time_2 = ros::Time::now();
    while(ros::ok() && isDoingMission){
        ...
        if(gate_passed == list_hgate[hgate_iter]){
            ...
            if(list_hgate.back() == (TOTAL_GATE -1)){
                ...
                if(ros::Time::now()-firstPassedLastGate > ros::Duration(4)){
                    hold_z = 2.0;
        ...
        if(!isLookingForHgate && gate_passed > 1 && landingPad.is_exist ){
            ...
            if(isFirstCenteringLand){
                isFirstCenteringLand=false;
                firstLandingCentering = ros::Time::now();
            }
            // centering
            handleLandingPad(vel, curYaw, 2.0, false);
            
            // cek kalo udah pas center pake threshold
            if(isCenteredStatic(landingPad.x, landingPad.y, camera_width/2, camera_height/2, current_pose.position.z*65.0)){
                soero::Log::Info("DEKET LANDING PAD CUY, TURUN SKUY");
                isDoingMission = false;
            }

            // nyegah centering kelamaan > 1 detik
            if(ros::Time::now() - firstLandingCentering > ros::Duration(0.5)){
                isDoingMission=false;// dah dah, turun 
                soero::Log::Info("centering dah 0.5 detikan, turun ae lah lur");
            }
            // isDoingMission = false;
        }
        // Cek horizontal gate                                  // check for false positive
            ...
            if(current_pose.position.z >= hold_z - 0.2){
                handleHorizontalGate(vel, curYaw);
            }
            
            // cek udah centered or no
            if(isCenteredRatioHrz(camera_width/2, camera_height/2,  horizontalGate, final_ratio_x, final_ratio_y)){
        ...
        // cek ada vertical gate ga? dan hanya jika ga lagi nyari hgate
            ...
            vel.linear.z = mypid_z_vision_gate.calculate(camera_height/2, verticalGate.y);
            hold_z = current_pose.position.z;
            ...
            if(isCenteredRatio(camera_width/2, camera_height/2, verticalGate)){
                ...
                // // hold_z = 1.35;
                soero::Log::Info("centered");
                while(ros::ok() && ros::Time::now() - start_time < ros::Duration(2)){
                    soero::Log::Info("dalam looping");
                    if(upperLine.is_exist){
                        soero::Log::Info("ada garis atas");
                        handleLine(vel, upperLine, line_vel_x_multiplier[isLookingForHgate]);
                    }
                    ...
                    // multipler(belok lebih tajam) agar tidak nabrak gate kampret
                    vel.linear.y *= 1.3;
                    vel.angular.z *= 1.3;
                ...
                if(!verticalGate.is_exist){
                    while(ros::ok() && ros::Time::now() - start_time_2 < ros::Duration(1) && gate_passed < list_hgate[hgate_iter]-1){
                        soero::Log::Info("dalam looping 2");
                        if(upperLine.is_exist){
                            soero::Log::Info("ada garis atas");
                            handleLine(vel, upperLine, line_vel_x_multiplier[isLookingForHgate]);
                        }
    ...
    soero::Log::Info("MISSION SUCCESS GENERAL!");
    double total_time = (ros::Time::now() - start_flight_time).toSec();
    std::string encrypted_time = soero::EncryptTime(total_time);
    soero::Log::Info("Token %s", encrypted_time.c_str());
