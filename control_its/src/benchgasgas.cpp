...
bool isCenteredRatio(double cx, double cy, 
    ...
    return isClose(object.x, cx, tx*0.45) && isClose(object.y, cy, ty*0.45);
    // return isClose(object.x, cx, tx/1.0) && isClose(object.y, cy, ty/1.0);
}
...
//                    MATH STUFF                                        //
//=======================================================================
constexpr double vx_max =1.4;                     // velocity maximum when line is align
constexpr double vx_min = 0.20;                    // minimum velocity when angular z exceed vaz_max
...
constexpr double area_min_v = 0.8; // max velocity when gate area is == area_min | jangan 0.5 berbahaya suka gagal-->ditambal pakek if else room top h bottom h gate
...
void handleLine(geometry_msgs::Twist &vel, vision::RotatedRect &line_guide, double mult_x=1.0){
    ...
    // maju mundur
    vel.linear.x = line_activation_x(vel.angular.z, mult_x)+0.15;
    // kiri kanan
...
void handleLandingPad(geometry_msgs::Twist &vel, double hold_yaw, double hold_z, bool withTurun){
    ...
    vel.linear.y = calc_vel_based_vision_z(landingPad.x, camera_width, max_meter, mypid_y_landing, 'y')*1.5;        
    vel.linear.x = calc_vel_based_vision_z(landingPad.y, camera_height, max_meter*(camera_height/camera_width), mypid_x_landing, 'x')*1.5;//agak majuan sikit, biar masuk
...
int main(int argc, char **argv)
{
    ...
    // confirm flight
    soero::Log::Info("You sure want to fly with currrent setting?|gas");
    ...
    while(ros::ok() && early_off){
        ...
        if(list_hgate.size() > 0){
            if(list_hgate[0] != 0){
                // bukan horizontal pertama
                if(verticalGate.is_exist){
                    handleVerticalGate(vel);
                    if(ros::Time::now()-existGuideTime > ros::Duration(1.0)){
                        early_off=true;
    ...
    */
    ...
    double threshold_vgateSide =9.0;
    bool doHold_Z = true;
    ...
    while(ros::ok() && isDoingMission){
        ...
        if(gate_passed == list_hgate[hgate_iter]){
            ...
            if(list_hgate.back() == (TOTAL_GATE -1)){
                // we have to add 5 secs delay until the drone hold_z to 2 -> cobak kalo udah jaraknya D cm an
                lastGatePos.position.z = current_pose.position.z;// biar z ga ngaruh, pinginnya x,y aja
                double dis_after_pass = calc_distance_to_wp(lastGatePos);
                if(dis_after_pass > 1.0){
                    hold_z = 2.0;
                    soero::Log::Info("time to detect landing pad, hold to %fm", hold_z);
                }
                soero::Log::Info("distance to last horgate: %f", dis_after_pass);
            }
        ...
        // Cek horizontal gate                                  // check for false positive
            ...
            if(isCenteredRatio(camera_width/2, camera_height/2,  horizontalGate, final_ratio_x, final_ratio_x)){
        ...
        // cek ada vertical gate ga? dan hanya jika ga lagi nyari hgate
            ...
            handleVerticalGate(vel); 
            if(isCenteredRatio(camera_width/2, camera_height/2, verticalGate) && isClose(verticalGateHelper.data[0], verticalGateHelper.data[1], threshold_vgateSide)){
    ...
    soero::Log::Info("Token %lf", total_time);
