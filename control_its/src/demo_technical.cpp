...
#include "../../vision_its/include/vision_its/XY_cor_msg.h"
...
PID mypid_x_back(dt, max_val+0.5, min_val-0.5, 0.750, 0.100, 0.00000);
PID mypid_x_back_far(dt, max_val+1.5, min_val-1.5, 1.5, 0.50, 0.00);
PID mypid_y_back(dt, max_val, min_val, 0.750, 0.300, 0.00000);
PID mypid_y_pseudo(dt, max_val-0.5, min_val+0.5, 0.30, 0.010, 0.00000);
PID mypid_y(dt, max_val*0.25, min_val*0.25, 0.7, 0.001, 0.000);
PID mypid_z(dt, max_val, min_val, Kp+0.39, Kd+0.02, 0);
PID mypid_z_takeoff(dt, max_val+0.5, min_val-0.5, 1, 0.155, 0.055);
PID mypid_z_landing(dt, max_val+1.5, min_val-1.5, 1, 0.155, 0.055);
PID mypid_z_vision_gate(dt, max_val, min_val, 0.30, 0.0101, 0);
PID mypid_y_vision_gate(dt, max_val, min_val, 0.30, 0.0101, 0);
...
void sonar_cb(sensor_msgs::RangeConstPtr msg){
    sonar = *msg;
}
...
void take_off(geometry_msgs::Twist *vel, ros::Publisher *cmd_pub, bool *takeoff, ros::Time *waktu, double initHeight){
    ...
    (*vel).linear.x = 0;
    (*vel).linear.y = 0;
    (*vel).linear.z = mypid_z_takeoff.calculate(initHeight+0.2, current_pose.position.z);
    (*cmd_pub).publish(*vel);
    if (current_pose.position.z < initHeight){
        *waktu = ros::Time::now();
        ROS_INFO("Taking off! | z: %.6f", current_pose.position.z);
    }
    else if (ros::Time::now() - (*waktu) > ros::Duration(0.5)){
        *takeoff = true;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber gas_sub = nh.subscribe<std_msgs::Bool>
        ("/gas", 5, gas_cb);
    ros::Subscriber sonar_sub = nh.subscribe<sensor_msgs::Range>
        ("/drone/sonar", 5, sonar_cb);
    //landing api
    ...
    for(counter = 0; counter < 3 && ros::ok(); counter++){
        ...
        while (ros::ok() && !back_to_school)
        {
            if(!there_is_gate.data && !centering && !lock_maju && !doneFirstTime){
                ...
                // go_forward(&vel, 1.1);
                vel.linear.x = 1.5*(abs(initGateCor.x - 320.0) < 250);
                //lock in 2 meter
                ...
                vel.linear.x = 0.000;
            //Wait for gas signal
                if(greenLight.data || sonar.range <= 0.5){
                    //PASS THE GATE
        ...
        while(ros::ok && back_to_school){
            ...
            //OK COUNTER == 2 BERARTI KE ABOVE PAD
                ...
                vel.linear.x = mypid_x_back_far.calculate(current_pose.position.y, above_pad_pose.position.y);
                // vel.linear.z = -2;
                vel.linear.z = mypid_z_landing.calculate(above_pad_pose.position.z, current_pose.position.z) * ((abs(above_pad_pose.position.y - current_pose.position.y) < 0.75));
                back_to_school = abs(current_pose.position.z) > 0.5; 
                if(!back_to_school){
                    land_pub.publish(empty);
                }
    ...
    double total_time = t.toSec();
    printf("========Total time: %.2lf========", t.toSec());
    return 0;
}