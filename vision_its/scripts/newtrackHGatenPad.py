...
from vision_its.msg import RotatedRect
...
class LandingVision:
  ...
  def __init__(self):
    ...
    self.rate.sleep()
    rospy.loginfo("Streaming landing vision| new")
  ...
  def detect_contour(self, mask, area_threshold, draw_image, isPad):
    ...
    if(len(contours) > 0):
        ...
        if(cv2.contourArea(max_area_cnt) > area_threshold):
          if isPad:
            # print(f"pad is exist {randint(0, 255)}")
            ...
            # cv2.putText(draw_image, f'{cv2.contourArea(max_area_cnt):.4f}', (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            # print(f'cnt area pad: {cv2.contourArea(max_area_cnt)}')
          else:
  ...
  def callback(self,data):
    ...
    self.detect_contour(mask_gate, 3000, cv_image, False)
    self.detect_contour(mask_pad, 9000, cv_image, True)
